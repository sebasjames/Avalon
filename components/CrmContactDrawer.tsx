      ...
      >
        <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900">{contact.company}</h2>
              <p className="text-sm text-slate-500 font-medium">{contact.name}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          ...
          <section>
            ...
            <div className="space-y-3 bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
              <div>
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Contacto 1</div>
              ...
              <div className="flex flex-col gap-2 mt-3 bg-slate-50/50 p-2.5 rounded-lg border border-slate-100">
                  <input type="text" placeholder="Hobbie / Intereses" value={contact.hobby || ''} onChange={(e) => updateContact(contact.id, { hobby: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full" />
                  <input type="date" title="Fecha de cumpleaños" value={contact.birthday || ''} onChange={(e) => updateContact(contact.id, { birthday: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full text-slate-500" />
                  <textarea placeholder="Observaciones..." value={contact.observations || ''} onChange={(e) => updateContact(contact.id, { observations: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full h-12 resize-none" />
              </div>
              ...
              <div className="pt-3 mt-1 border-t border-slate-100">
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-sm font-medium text-slate-700">
                    {contact.contact2 || <span className="text-slate-400 italic font-normal">No asignado</span>}
                  </span>
                  <div className="flex items-center gap-2 text-sm">
                    <Phone className="w-3.5 h-3.5 text-slate-400" />
                    <span className="text-slate-600">
                      {contact.phone2 || <span className="text-slate-400 italic">No asignado</span>}
                    </span>
                  </div>
                  <div className="flex flex-col gap-2 mt-2 bg-slate-50/50 p-2.5 rounded-lg border border-slate-100">
                      <input type="text" placeholder="Hobbie / Intereses" value={contact.hobby2 || ''} onChange={(e) => updateContact(contact.id, { hobby2: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full" />
                      <input type="date" title="Fecha de cumpleaños" value={contact.birthday2 || ''} onChange={(e) => updateContact(contact.id, { birthday2: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full text-slate-500" />
                      <textarea placeholder="Observaciones..." value={contact.observations2 || ''} onChange={(e) => updateContact(contact.id, { observations2: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full h-12 resize-none" />
                  </div>
                </div>
              </div>
              ...
              <div className="pt-3 mt-1 border-t border-slate-100">
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-sm font-medium text-slate-700">
                    {contact.contact3 || <span className="text-slate-400 italic font-normal">No asignado</span>}
                  </span>
                  <div className="flex items-center gap-2 text-sm">
                    <Phone className="w-3.5 h-3.5 text-slate-400" />
                    <span className="text-slate-600">
                      {contact.phone3 || <span className="text-slate-400 italic">No asignado</span>}
                    </span>
                  </div>
                  <div className="flex flex-col gap-2 mt-2 bg-slate-50/50 p-2.5 rounded-lg border border-slate-100">
                      <input type="text" placeholder="Hobbie / Intereses" value={contact.hobby3 || ''} onChange={(e) => updateContact(contact.id, { hobby3: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full" />
                      <input type="date" title="Fecha de cumpleaños" value={contact.birthday3 || ''} onChange={(e) => updateContact(contact.id, { birthday3: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full text-slate-500" />
                      <textarea placeholder="Observaciones..." value={contact.observations3 || ''} onChange={(e) => updateContact(contact.id, { observations3: e.target.value })} className="text-[11px] bg-white border border-slate-200 rounded p-1.5 focus:outline-none focus:border-indigo-400 w-full h-12 resize-none" />
                  </div>
                </div>
              </div>
          ...
          {contact.purchaseHistory && (
            <section>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[10px] font-bold text-slate-400 uppercase">Evolución Comercial (Últimos 4 Meses)</p>
                  <div className="flex items-center gap-1 bg-slate-100 p-0.5 rounded-lg border border-slate-200">
                    <button onClick={() => setChartType('bar')} className={`p-1 rounded ${chartType === 'bar' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
                      <BarChart className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => setChartType('line')} className={`p-1 rounded ${chartType === 'line' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
                  ...
                  {chartType === 'bar' ? (
                    ...
                    <div className="w-full h-full flex flex-col justify-between">
                      <div className="flex-1 relative w-full pt-4">
                        <svg className="w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 100 100">
                          <polyline
                            ...
                            points={contact.purchaseHistory.evolution.map((p, i) => {
                              const maxAmount = Math.max(...contact.purchaseHistory!.evolution.map(e => e.amount));
                              return `${(i / (contact.purchaseHistory!.evolution.length - 1)) * 100},${100 - (p.amount / maxAmount) * 100}`
                            }).join(' ')}
                          />
                          {contact.purchaseHistory.evolution.map((p, i) => {
                            const maxAmount = Math.max(...contact.purchaseHistory!.evolution.map(e => e.amount));
                            return (
                              <g key={i} className="group cursor-pointer">
                                <circle
                                  cx={(i / (contact.purchaseHistory!.evolution.length - 1)) * 100}
                                  cy={100 - (p.amount / maxAmount) * 100}
                                  r="4"
                                  fill="white"
                                  stroke="currentColor"
                                  strokeWidth="2"
                      ...
                      <div className="flex justify-between w-full mt-2">
                        {contact.purchaseHistory.evolution.map((point, idx) => (
                          <span key={idx} className="text-[10px] font-bold text-slate-500 uppercase flex-1 text-center">{point.month}</span>
                        ))}
                      </div>
                    </div>
                  )}
            ...
            <div className="space-y-3 bg-slate-50 border border-indigo-100 rounded-xl p-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={contact.hobby || ''}
                  onChange={(e) => updateContact(contact.id, { hobby: e.target.value })}
                  className="w-full text-sm border border-slate-200 rounded-md p-1.5 bg-white focus:outline-none focus:border-indigo-400"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full text-sm border border-slate-200 rounded-md p-1.5 bg-white focus:outline-none focus:border-indigo-400 resize-none h-16"
                />
              </div>
            </div>
          </section>
                ...
                if (isActivity) {
                  ...
                  return (
                    <div key={activity.id} className="relative pl-6">
                      <div className={`absolute -left-[17px] top-1 p-1.5 rounded-full border-2 border-white ${activity.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-600' :
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-1 cursor-pointer hover:bg-slate-50 p-1 -ml-1 rounded transition-colors" onClick={() => setIsCommercialRulesExpanded(!isCommercialRulesExpanded)}>
                <Tags className="w-4 h-4 text-indigo-500" />
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                  Condiciones Comerciales
                </h3>
                {isCommercialRulesExpanded ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />}
              </div>
              {!isEditingCommercialRules ? (
                <button
                  onClick={() => { setIsCommercialRulesExpanded(true); setIsEditingCommercialRules(true); }}
