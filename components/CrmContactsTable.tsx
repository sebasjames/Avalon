    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
        ...
        <div className="flex items-center gap-2 border-l border-slate-200 pl-4 relative">
            ...
            <button onClick={() => setIsExcelModalOpen(true)} className="flex items-center gap-1 px-3 py-1.5 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-md text-xs font-bold hover:bg-emerald-100 transition mr-2">
              ...
              {currentContacts.map(contact => {
                ...
                >
                  ...
                  {visibleColumns.includes('status') && renderSelectCell('status', 
                    [{value:'PROSPECTO', label:'Prospecto'}, {value:'VINCULADO', label:'Vinculado'}, {value:'INACTIVO', label:'Inactivo'}],
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
                      ...
                      {contact.status === 'VINCULADO' ? 'Vinculado' : contact.status === 'PROSPECTO' ? 'Prospecto' : 'Inactivo'}
                    </span>
                  )}
