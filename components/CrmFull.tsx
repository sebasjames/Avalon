...
export const CrmFull: React.FC = () => {
  ...
  const handleCreateRegistro = () => {
    ...
    const initialStatus = newPayload.status || (newRegistroType === 'DEAL' ? 'PROSPECTO' : 'VINCULADO');
    ...
    if (newRegistroType === 'DEAL') {
      const nd: CrmDeal = {
        id: `D-NEW-${Date.now()}`,
        title: `Prospecto - ${company}`,
        contactId: contactId,
        company: company,
        value: 0,
        stage: 'LEAD',
  ...
  return (
    <div className="p-4 md:p-8 w-full min-h-full relative flex flex-col flex-1">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-6">
        <div className="flex items-center gap-1 bg-slate-100/50 p-1 rounded-xl w-fit border border-slate-200/60 flex-wrap">
          {[
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="U-CARLOS">Embudo de Carlos</option>
                    <option value="U-MARIA">Embudo de María</option>
                  </select>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm overflow-y-auto py-10">
            <div className="bg-white rounded-2xl w-full max-w-2xl p-6 shadow-2xl my-auto max-h-full overflow-y-auto">
              ...
              <div className="flex gap-2 mb-6 border-b border-slate-200 pb-4">
                ...
                <button onClick={() => setNewRegistroType('DEAL')} className={`px-4 py-2 text-sm rounded-lg transition-colors ${newRegistroType === 'DEAL' ? 'bg-indigo-600 text-white font-bold shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>Nuevo Prospecto</button>
              </div>
              ...
              <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select 
                                className="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 ring-indigo-500 outline-none bg-white" 
                                onChange={(e) => setNewPayload({...newPayload, status: e.target.value})}
                                value={newPayload.status || (newRegistroType === 'DEAL' ? 'PROSPECTO' : 'VINCULADO')}
                            >
                                <option value="PROSPECTO">Prospecto (Oportunidad Temprana)</option>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 mb-1">Hobbie / Intereses</label>
                            <input className="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 ring-indigo-500 outline-none" placeholder="Ej. Fútbol, lectura" onChange={(e) => setNewPayload({...newPayload, hobby: e.target.value})} />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-slate-500 mb-1">Fecha de Cumpleaños</label>
                            <input type="date" className="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 ring-indigo-500 outline-none" onChange={(e) => setNewPayload({...newPayload, birthday: e.target.value})} />
                        </div>
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-slate-500 mb-1">Observaciones</label>
                        <textarea className="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 ring-indigo-500 outline-none h-20 resize-none" placeholder="Información adicional sobre el contacto..." onChange={(e) => setNewPayload({...newPayload, observations: e.target.value})}></textarea>
                    </div>
