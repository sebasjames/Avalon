...
import { PackageSearch, CheckCircle2, MapPin, Save, Lock, XCircle, AlertTriangle } from 'lucide-react';
...
export const InventarioTransito: React.FC = () => {
    ...
    // assignments: { [receiptId]: { [sku]: locationName } }
    ...
    const handleAssign = (receiptId: string, sku: string, location: string) => {
        setAssignments(prev => ({
            ...prev,
            [receiptId]: {
                ...(prev[receiptId] || {}),
    ...
    const handleSave = (receiptId: string) => {
        ...
        const receiptAssignments = assignments[receiptId] || {};
        const unassigned = receipt.items.some(item => !receiptAssignments[item.sku]);
        
        if (unassigned) {
            alert('Aún hay productos sin bodega asignada. Por favor, asigna una bodega a cada producto.');
            return;
        }
    ...
    const confirmSave = () => {
        ...
        // Convert { sku: 'Centenario' } to { sku: { 'Centenario': qty } }
        const distributionData: Record<string, Record<string, number>> = {};
        receipt.items.forEach(item => {
            const loc = receiptAssignments[item.sku];
            if (loc) {
                distributionData[item.sku] = {
                    [loc]: item.totalLiters
                };
            }
        });
        ...
        <div className="p-8 max-w-7xl mx-auto">
            <div className="mb-8">
                ...
                <p className="text-slate-500 mt-2">
                    Gestiona la mercancía recibida y distribúyela físicamente a las diferentes bodegas antes de liberarla para la venta.
                </p>
            </div>
            ...
            {transitReceipts.length === 0 ? (
                ...
                <div className="space-y-8">
                    {transitReceipts.map(receipt => {
                        const receiptAssignments = assignments[receipt.id] || {};
                        const allAssigned = receipt.items.length > 0 && receipt.items.every(i => !!receiptAssignments[i.sku]);
                                    ...
                                    <table className="w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {receipt.items.map((item, idx) => {
                                                const assignedLocation = receiptAssignments[item.sku];
                                                return (
                                                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <td className="p-4 text-right font-mono font-bold text-slate-700">
                                                            {item.totalLiters.toLocaleString()} <span className="text-xs text-slate-400 font-sans font-normal">{item.capacity || 'UOM'}</span>
                                                        </td>
                                                        <td className="p-4 text-center">
                                                            <select
                                                                value={assignedLocation || ''}
                                                                onChange={(e) => handleAssign(receipt.id, item.sku, e.target.value)}
