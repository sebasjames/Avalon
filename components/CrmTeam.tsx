              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <div>
                      <span className="text-slate-500 font-medium block text-xs">Recaudado</span>
                      <span className={`font-bold ${pct >= 95 ? 'text-emerald-600' : 'text-amber-500'}`}>${won.toLocaleString('es-CO')} COP</span>
                    </div>
                    <div className="text-right">
                      <span className="text-slate-500 font-medium block text-xs">Meta global</span>
                      <span className="font-bold text-slate-800">${user.quota.toLocaleString('es-CO')} COP</span>
                    </div>
                  </div>
                  <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mt-1">
                    <div className={`${pct >= 95 ? 'bg-emerald-500' : 'bg-amber-500'} h-2 rounded-full transition-all`} style={{ width: `${pct}%` }} />
                  </div>
                </div>

                <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                  <div>
                    <p className="text-[10px] uppercase font-bold text-slate-400">Embudo Activo</p>
                    <p className="font-bold text-indigo-600">${pipeline.toLocaleString('es-CO')} COP</p>
                  </div>
                  <div className="text-right">
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              {(() => {
                ...
                const prospectosIngresados = uContacts.filter(c => c.status === 'PROSPECTO').length;
                const prospectosGestionados = uContacts.filter(c => activities.some(a => a.contactId === c.id && a.status === 'COMPLETED')).length;
                const prospectosConvertidos = uContacts.filter(c => c.status === 'VINCULADO').length;
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-bold text-slate-900">{prospectosIngresados}</p>
                        </div>
                        <div>
                          <p className="text-[10px] uppercase font-bold text-slate-400">Prospectos Gestionados</p>
                          <p className="text-lg font-bold text-slate-900">{prospectosGestionados}</p>
                        </div>
                        <div>
                          <p className="text-[10px] uppercase font-bold text-slate-400">Prospectos Convertidos</p>
                          <p className="text-lg font-bold text-emerald-600">{prospectosConvertidos}</p>
                        </div>
                        <div>
              ...
              <div>
                <h4 className="font-semibold text-slate-800 flex items-center gap-2 mb-4"><BarChart2 className="w-4 h-4 text-indigo-500"/> Embudo Principal</h4>
                <div className="space-y-3">
                  {deals.filter(d => d.ownerId === selectedUser.id && d.stage !== 'CLOSED_WON' && d.stage !== 'CLOSED_LOST').sort((a,b) => b.value - a.value).slice(0, 5).map(deal => (
                    <div key={deal.id} className="p-3 border border-slate-100 rounded-lg hover:bg-slate-50">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-sm text-indigo-600">${deal.value.toLocaleString('es-CO')} COP</span>
                      </div>
